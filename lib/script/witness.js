...
class Witness extends Stack {
  ...
  /**
   * Get the Taproot annex if present.
   * @returns {Buffer|null} Taproot annex.
   */
  ...
  /**
   * Get the Taproot spend type.
   * From bip-taproot and bip-tapscript:
   *   Bit 0 is set if an annex is present
   *   Bit 1 is set if the script path is being spent
   * @returns {Number}
   */
  ...
  getSpendType() {
    ...
    // "...this last element is called annex a and is removed from the stack."
    // "If there is exactly one element left... key path spending is used."
    // "If there are at least two elements left, script path spending is used."

    const itemsMinusAnnex = this.items.length - (annex ? 1 : 0);

    if (itemsMinusAnnex >= 2)
      spendType |= 1 << 1;
  ...
  /**
   * Get the Tapscript tapleaf (script)
   * @returns {Buffer|null} Tapleaf script
   */

  getTapleaf() {
    const spendType = this.getSpendType();

    if (~spendType & (1 << 1))
      return null;

    const itemsMinusAnnex = this.items.length - (spendType & (1 << 0) ? 1 : 0);

    assert(itemsMinusAnnex >= 2);

    return this.items[itemsMinusAnnex - 2];
  }

  /**
   * Get the Tapscript control block
   * @returns {Buffer|null} Tapleaf script
   */
  ...
  getControlBlock() {
    ...
    return this.items[itemsMinusAnnex - 1];
  }

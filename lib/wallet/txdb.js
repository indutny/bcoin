...
class Details {
  ...
  /**
   * Calculate change to wallet balance.
   * @returns {Amount}
   */
  ...
  /**
   * Calculate change to account balance.
   * @returns {Map}
   */
  ...
  getAccountValues() {
    ...
    for (const input of this.inputs) {
      if (input.path) {
        acctBalance = out[input.path.account];
        if (!acctBalance)
            out[input.path.account] = input.value * -1;
        else
            out[input.path.account] = acctBalance - input.value;
      }
    }

    for (const output of this.outputs) {
      if (output.path) {
        acctBalance = out[output.path.account];
        if (!acctBalance)
            out[output.path.account] = output.value;
        else
            out[output.path.account] =  acctBalance + output.value;
      }
    }
  ...
  toJSON(network, height) {
    ...
    const walletValue = this.getWalletValue();
    const accountValues = this.getAccountValues();

...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/fee', async (req, res) => {
      ...
      const rate = await this.wdb.feeRate;
      res.json(200, {'rate': rate});
    });

    // Set global tx fee rate
    this.post('/fee', async (req, res) => {
      if (!req.admin) {
        res.json(403);
        return;
      }
      ...
      const valid = Validator.fromRequest(req);
      const rate = valid.ufixed('rate', 8);

      enforce(rate, 'rate is required');
